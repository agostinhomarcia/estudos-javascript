...
const lastIndex = array.length - 1
lastIndex: 3 - 1 = 2
index (0) === lastIndex(2): result  0 === 2  false
firstLet = firstLet(0) + lastIndex(2) = 0 + 2   firstLet = 2
secondLet = secondLet(3) * index(0) result  3 * 0 = 0


2ª execução
const lastIndex = array.length - 1
lastIndex: 3 - 1 = 2
index(1) === lastIndex(2):  result 1 === 2 false
firstLet = firstLet(2) + lastIndex(2) = 2 + 2    firstLet = 4
secondLet = secondLet(0) * index(1) = result 0 * 1 = 0

3ª execução
const lastIndex = array.length - 1
lastIndex: 3 - 1 = 2
index(2) === lastIndex(2): result 2 === 2 true
thirdLet = 'laranja
firstLet = firstLet(4) + lastIndex(2) = 4 + 2   firstLet = 6
secondLet = secondLet(0) * index(2) = result 0 * 2 = 0
 */
...
letters.forEach((letter) => {
  lettersInUppercase.push(letter.toUpperCase());
});
...
const customers = [
  {
    ...
    orders: [
      {
        id: 59,
        product: "Camiseta Comfort Em Algodão Com Estampa Route 66",
        price: 29.9,
      },
  ...
  },
  {
    ...
    orders: [
      ...
      {
        id: 645,
        product: "Bolsa Transversal Com Alça De Corrente",
        price: 59.9,
      },
      {
        id: 491,
        product: "Pijama Americano Curto Em Viscolycra Com Viés Contrastante",
        price: 159.9,
      },
    ],
  },
...
customers.forEach((customer) =>
  priceMessages.push(`${customer.name} deve pagar R$ 119.80 `)
);
...
forEach([1, 2, 3], function (item, index, array) {
  loopMessages.push(`${item} é o ${index + 1}º item do array ${array}`);
});

console.log(loopMessages);
// resultado: ['1 é o 1º item do array [1, 2, 3]', '2 é o 2º item do array [1, 2, 3]', '3 é o 3º item do array [1, 2, 3]']

const getMultipler = (multipler) => {
  return function (aNumber) {
    return multipler * aNumber;
  };
};

const double = getMultipler(2);
const triple = getMultipler(3);
const quadruple = getMultipler(4);

console.log(double(9), triple(3), quadruple(5));
