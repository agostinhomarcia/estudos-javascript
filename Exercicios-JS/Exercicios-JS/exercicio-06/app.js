...
for (let i = 0; i < crazyArray.length; i++) {
  const typeOfItem = typeof crazyArray[1];
  let isItemANumber = typeOfItem === "number";
  let isItemABoolean = typeOfItem === "boolean";
...
const lastOdd = oddNumbers[oddNumbers.length - 1];
const lastEven = evenNumbers[evenNumbers.length - 1];

const oddNumbersString = oddNumbers.slice(0, -1).join(", ");
const evenNumbersString = evenNumbers.slice(0, -1).join(", ");

const sentence = `Odd numbers: ${oddNumbersString} and ${lastOdd}. Even numbers: ${evenNumbersString} and ${lastEven}.`;
