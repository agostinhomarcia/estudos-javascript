...
const crazyNewArray = crazyNumbers.reduce((accumulator, crazyNumber) => {
  if (crazyNumber < 501) {
    accumulator += 1;
  }
  return accumulator;
}, 0);
console.log(crazyNewArray);
...
const newArrayMovies = tarantinoMovies.filter((movie) => movie.release < 2000);
...
const newShows = tvShows.map((tvShow) => tvShow.name);
