...
const getFormattedTime = (template, [hours, minutes, seconds]) => {
  const formattedTime = formatTimeUnits([hours, minutes, seconds]);
...
const createClock = ({ template }) => {
  ...
  clockObj.render = () => {
    const formattedTime = getFormattedTime(template, getTime());
    console.log(formattedTime);
  };
  ...
  clockObj.start = () => {
    ...
    clockObj.render();
    clockObj.timer = setInterval(() => clockObj.render(), oneSecond);
  };

  clockObj.stop = () => {
    clearInterval(clockObj.timer);
  };
...
const createExtendedClock = ({ template, precision = 1000 }) => {
  const extendedClockObj = createClock({ template });

  extendedClockObj.start = () => {
    extendedClockObj.render();
    extendedClockObj.timer = setInterval(
      () => extendedClockObj.render(),
      precision
    );
  };

  return extendedClockObj;
};
...
const getUrl = (currency) =>
  ` https://v6.exchangerate-api.com/v6/9d837374e80b9074498b27d0/latest/${currency}`;

const getErrormessage = (errorType) =>
  ({
    "unsupported-code": "A moeda não existe em nosso banco de dados",
    "base-code-only-on-pro":
      "Informações de moedas que não sejam USD ou EUR só podem ser acessadas",
    "malformed-request":
      "o endpoint do seu request precisa seguir a seguinte estrutura á seguir",
    "invalid-key": " A have da API não é válida",
    "quota-reached":
...
const fetchExchangeRate = async (url) => {
  try {
    ...
    if (!response.ok) {
      throw new Error(
        "Sua conexão falhou , não foi possível obter as informçãoes"
      );
    }
