...
const numbers3 = [10, 67, 99, 45];
const numbers4 = [1, 2, 3, 4];

const sum = (...params) => {
  return params.reduce((acc, number) => acc + number, 0);
};

// console.log(sum(...numbers)); // deve retornar 200

// console.log(sum(...numbers2)); // deve retornar 30
// console.log(sum(...numbers3)); // deve retornar 221

// console.log(sum(...numbers4)); // deve retornar 10
...
const getIndexesOfCharacter = (string, character) =>
  [...string].reduce(
    (acc, item, index) => (item === character ? [...acc, index] : acc),
    []
  );
